    ...
    <SafeArea topBGC="#58abdd">
      <FullHeight>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-evenly', padding: '15px 0', borderBottom: '5px solid #e4e4e4' }}>
          <Acon icon="User" title='新增粉丝' color={'#666'} outerStyle={{ gap: 5, flexDirection: 'column' }} />
          <Acon icon="Comment" title='评论回复' color={'#666'} outerStyle={{ gap: 5, flexDirection: 'column' }} />
          <Acon icon="Heart" title='收藏' color={'#666'} outerStyle={{ gap: 5, flexDirection: 'column' }} />
          <Acon icon="ThumbsUp" title='收到的赞' color={'#666'} outerStyle={{ gap: 5, flexDirection: 'column' }} />
          {/* <Acon icon="System" title='系统消息' color={'#666'} outerStyle={{ gap: 5, flexDirection: 'column' }} /> */}
        </div>
        <FullHeightAuto>
          <PageList
            items={local.list}
