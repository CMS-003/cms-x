...
export default function User({ template, id }) {
  ...
  const local = useLocalObservable(() => ({
    ...
    setValue(k, v) {
      switch (k) {
        ...
        case 'compilation': local.compilation = v; break;
  ...
  const getCompilation = useCallback(async () => {
    try {
      const result = await apis.getCompilationDetail(id)
      if (result && result.code === 0) {
        local.setValue('compilation', result.data.item);
  ...
  const getData = useCallback(async () => {
    try {
      ...
      const resp = await apis.getCompilationResources(id, { page: local.page, size: 20 });
      if (resp.code === 0) {
      ...
      return (
        <SafeArea>
          ...
          <FullWidth style={{ alignItems: 'stretch', padding: 10 }}>
            ...
            <FullWidthAuto style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <div style={{ fontSize: 18 }}>
                {local.compilation?.title}
                <div style={{ fontSize: 12, color: '#999' }}>{local.compilation?.desc}</div>
              </div>
              <div style={{ fontSize: 12, color: '#999', marginTop: 15 }}>
                创建于{dayjs(local.compilation?.createdAt).format('YYYY-MM-DD')}
              </div>
            </FullWidthAuto>
            {store.user.isLogin &&
              <Button
              ...
              >{local.user.counted.followed ? '取消订阅' : '订阅'}</Button>}
          </FullWidth>
