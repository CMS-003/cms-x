...
export default function User({ item }) {
  ...
  return <Observer>{() => (
    <ItemWrap style={{ flexDirection: 'row', alignItems: 'center' }} onClick={() => {
      ...
      <div style={{ width: 100, height: 70, backgroundImage: `url(${(picture || '/images/poster/nocover.jpg')})`, backgroundSize: 'cover', backgroundRepeat: 'no-repeat', flexShrink: 0, flexGrow: 0, backgroundPosition: 'center center' }}></div>
      <div style={{ flex: 1 }}>
        <ItemTitle style={{ lineHeight: 1.2 }}>{item.title}</ItemTitle>
        <FullWidth style={{ fontSize: 12, color: 'grey', margin: '0 5px 5px 5px' }}>
          <span style={{ margin: '0 5px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>注册于 {readableTime((new Date(item.createdAt)))}</span>
